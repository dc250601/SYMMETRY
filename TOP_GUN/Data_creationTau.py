...
if __name__ == "__main__":
    idx = int(sys.argv[1])
    # Start idx ---> start idx + 512

    FILE_LIST = glob.glob("/pscratch/sd/d/diptarko/Tau/*/*.parquet*")[idx:(idx+512)]
    # FILE_LIST = [FILE_LIST[0]]

    NO_SAMPLES = sum(list(pq.ParquetFile(file).metadata.num_rows for file in FILE_LIST))

    data = h5py.File(f'/pscratch/sd/d/diptarko/SYMMETRY/TOP_GUN/assets/DATA/Tau_{idx}.h5', 'w')

    data_chunk_meta = data.create_dataset("meta", (NO_SAMPLES,12))
    data_chunk_jet = data.create_dataset("jet", (NO_SAMPLES,13,125,125))
        ...
        while(True):
            try:
                ...
                try:
                    ...
                    samples = p.shape[0]
                    im = np.array(np.array(np.array(p.iloc[:,9].tolist()).tolist()).tolist())
                    meta = np.array(p.drop(['X_jet'],axis = 1))
...
# data = h5py.File('/pscratch/sd/d/diptarko/SYMMETRY/TOP_GUN/assets/DATA/Tau.h5', 'r+')
# BATCH_SIZE = 4096
# CHANNELS = 13
